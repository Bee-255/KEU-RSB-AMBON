...
// ✅ Impor semua fungsi helper yang sudah dipindahkan
import { capitalizeWords, formatAngka, parseAngka, toRoman, formatTanggal } from '../lib/utils';
// ✅ Impor fungsi createPDF dari lib/pdfSppr.js
...
const Sppr = () => {
  ...
  const fetchSPPR = async () => {
    const { data, error } = await supabase.from("sppr").select("*, pangkat_kpa, pangkat_bendahara").order("created_at", { ascending: false });
    if (error) {
  ...
  const fetchKPA = async () => {
    const { data, error } = await supabase
      ...
      .select("*, pegawai(nama, pangkat, nrp_nip_nir, tipe_identitas)")
  ...
  const fetchBendahara = async () => {
    ...
    if (error) {
      ...
      setBendaharaList(formattedData);
    }
  };
  ...
  const fetchPengambil = async () => {
    const { data, error } = await supabase
      .from("pegawai")
      .select("nama, pangkat, nrp_nip_nir, tipe_identitas")
        ...
        <table className={pageStyles.table}>
          <thead className={pageStyles.tableHead}>
            <tr>
              ...
              <th style={{ width: "25%" }}>Jumlah Penarikan</th>
              <th style={{ width: "20%" }}>Nama Pengambil</th>
            </tr>
          </thead>
      ...
      <div className={pageStyles.detailContainer}>
        ...
        {selectedSppr ? (
          <div className={pageStyles.detailContent}>
            ...
            <div className={pageStyles.detailItem}>
              <div className={pageStyles.detailLabel}>Pangkat Pengambil</div>
              <div className={pageStyles.detailValue}>: {selectedSppr.pangkat_pengambil}</div>
            </div>
