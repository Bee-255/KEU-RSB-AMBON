...
const Sppr = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    jumlah_penarikan: 0,
    operator: "",
    status_sppr: "Baru", // ✅ Ubah nama kolom menjadi status_sppr dan default ke "Baru"
  });

  const [operatorName, setOperatorName] = useState("");
  const [userRole, setUserRole] = useState(""); // ✅ State baru untuk menyimpan role user
  ...
  const getLoggedInUser = async () => {
    ...
    if (user) {
      const { data: profile } = await supabase
        .from("profiles")
        .select("nama_lengkap, role") // ✅ Ambil juga kolom role
        .eq("id", user.id)
        .single();
      if (profile) {
        setOperatorName(profile.nama_lengkap);
        setUserRole(profile.role); // ✅ Set state role user
      }
    }
  };
  ...
  const fetchPejabatAndPegawai = async () => {
    const { data: kpaData, error: kpaError } = await supabase
    ...
    const { data: bendaharaData, error: bendaharaError } = await supabase
      .from("pejabat_keuangan")
      .select("*, pegawai(nama, pangkat, nrp_nip_nir, tipe_identitas, jabatan_struktural)")
    ...
    const { data: pengambilData, error: pengambilError } = await supabase
      .from("pegawai")
  ...
  const generateNomorSurat = async () => {
    ...
    setFormData((prevData) => ({
      ...
      status_sppr: "Baru", // ✅ Pastikan status di-reset saat membuat surat baru
    }));
  };
  ...
  const handleApprove = async () => {
    ...
    if (result.isConfirmed) {
      const { error } = await supabase
        .from('sppr')
        .update({ status_sppr: 'Disetujui' }) // ✅ Update kolom status_sppr
        .eq('id', selectedSppr.id);
      if (error) {
  ...
  const resetForm = () => {
    ...
    setFormData({
      ...
      jumlah_penarikan: 0,
      operator: operatorName,
      status_sppr: "Baru", // ✅ Pastikan status kembali ke "Baru" saat reset
    });
    setShowModal(false);
  };
            ...
            {paginatedSppr.length > 0 ? (
              paginatedSppr.map((sppr, index) => (
                ...
                >
                  ...
                  <td>{sppr.status_sppr}</td> {/* ✅ Tampilkan status_sppr */}
                </tr>
              ))
            ) : (
              <tr>
