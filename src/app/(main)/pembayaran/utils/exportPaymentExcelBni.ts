...
export const exportPaymentExcelBni = async (
    ...
    const filteredDetails = details.filter(d => 
        d.bank && d.bank.toUpperCase().trim() === 'BNI'
    );
    
    if (filteredDetails.length === 0) {
        showToast("Tidak ada detail pembayaran dengan Bank Negara Indonesia yang ditemukan untuk diunduh.", "warning");
        return;
    }
    ...
    try {
        ...
        worksheet.views = [{ 
            state: 'frozen', 
            ySplit: 3, // Bekukan baris header (Baris 1 dan Baris 2 kosong)
            showGridLines: true 
        }];
        ...
        // Gabungkan sel untuk label "TOTAL" dari A sampai D diubah menjadi A sampai C
        ...
        totalRow.eachCell(cell => {
            ...
            // Semua sel di baris ini di-center dulu
            cell.alignment = { vertical: 'middle', horizontal: 'center' };
            cell.border = { 
                top: {style:'thin'}, left: {style:'thin'}, 
                bottom: {style:'thin'}, right: {style:'thin'}
        ...
        // Kolom D ("TOTAL") harus align KANAN (agar dekat dengan nilai total)
