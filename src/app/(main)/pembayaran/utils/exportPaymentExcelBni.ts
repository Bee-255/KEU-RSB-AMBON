...
export const exportPaymentExcelBni = async (
    ...
    // 🚀 FILTERING: Ambil hanya data yang banknya adalah "BNI"
    const filteredDetails = details.filter(d => 
        d.bank && d.bank.toUpperCase().trim() === 'BNI'
    );
    
    if (filteredDetails.length === 0) {
        showToast("Tidak ada detail pembayaran dengan Bank BNI yang ditemukan untuk diunduh.", "warning");
        return;
    }

    try {
        const workbook = new ExcelJS.Workbook();
        const worksheet = workbook.addWorksheet("Lampiran Bank BNI");
        const filename = `Pembayaran_BNI_${payment.periode_pembayaran.replace(/\s/g, '_')}.xlsx`;
        
        // --- 1. SET JUDUL ---
        const titleRow = worksheet.addRow(["DAFTAR LAMPIRAN BANK BNI"]);
