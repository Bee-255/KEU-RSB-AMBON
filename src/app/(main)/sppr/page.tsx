...
const Sppr = () => {
  ...
  const fetchRekeningOptions = useCallback(async () => {
    try {
      ...
      // Format data dengan penanganan error yang lebih baik
      const formattedOptions: RekeningOption[] = rekeningData?.map((item: any) => {
        try {
          const kodeAkun = item.kode_akun_bank || '';
          let namaAkun = 'Akun Tidak Ditemukan';
          const nomorRekening = item.nomor_rekening || 'Nomor Tidak Ditemukan';

          // Handle berbagai kemungkinan struktur bas_akun
          if (item.bas_akun) {
            if (Array.isArray(item.bas_akun) && item.bas_akun.length > 0) {
              namaAkun = item.bas_akun[0]?.nama_akun || 'Akun Tidak Ditemukan';
            } else if (typeof item.bas_akun === 'object' && item.bas_akun.nama_akun) {
              namaAkun = item.bas_akun.nama_akun;
            }
          }

          const value = `${namaAkun} - ${nomorRekening}`;
          ...
          console.error("Error formatting rekening item:", err);
          return {
            value: 'Error - Format Data',
            label: 'Error - Format Data',
            kodeAkun: '000',
          };
        }
      }) || [];
  ...
  const fetchPejabatAndPegawai = useCallback(async () => {
    try {
      ...
      const formattedKpa: PersonType[] = kpaData?.map(item => ({
        nama: item.pegawai?.nama || "",
        pangkat: item.pegawai?.pangkat || "",
        tipe_identitas: item.pegawai?.tipe_identitas || null,
        nrp_nip_nir: item.pegawai?.nrp_nip_nir || null,
      ...
      const formattedBendahara: PersonType[] = bendaharaData?.map(item => ({
        nama: item.pegawai?.nama || "",
        pangkat: item.pegawai?.pangkat || "",
        tipe_identitas: item.pegawai?.tipe_identitas || null,
        nrp_nip_nir: item.pegawai?.nrp_nip_nir || null,
      ...
      const formattedPengambil: PersonType[] = pengambilData?.map(item => ({
        nama: item.nama || "",
        pangkat: item.pangkat || "",
        tipe_identitas: item.tipe_identitas || null,
        nrp_nip_nir: item.nrp_nip_nir || null,
  ...
  useEffect(() => {
    ...
    initializeData();
  }, [getLoggedInUser, fetchSPPR, fetchPejabatAndPegawai, fetchRekeningOptions]);
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
    ...
    setFormData(prevData => {
      ...
      if (name === "jumlah_penarikan") {
        ...
        newFormData.pangkat_pengambil = selectedPengambil ? 
          `${selectedPengambil.pangkat} ${selectedPengambil.tipe_identitas || ''} ${selectedPengambil.nrp_nip_nir || ''}`.trim() : "";
        ...
        newFormData.pangkat_kpa = selectedKpa ? 
          `${selectedKpa.pangkat} ${selectedKpa.tipe_identitas || ''} ${selectedKpa.nrp_nip_nir || ''}`.trim() : "";
        ...
        newFormData.pangkat_bendahara = selectedBendahara ? 
          `${selectedBendahara.pangkat} ${selectedBendahara.tipe_identitas || ''} ${selectedBendahara.nrp_nip_nir || ''}`.trim() : "";
  ...
  return (
    <div className={pageStyles.container}>
      ...
      <div className={pageStyles.buttonContainer}>
        <button 
          onClick={() => { resetForm(); fetchPejabatAndPegawai(); setShowModal(true); }} 
              ...
              <div className={pageStyles.formGroup}>
                ...
                >
                  ...
                  {kpaList.map((kpa, index) => (
                    <option key={index} value={kpa.nama}>{kpa.nama}</option>
                  ))}
                </select>
              </div>
              <div className={pageStyles.formGroup}>
              ...
              <div className={pageStyles.formGroup}>
                ...
                >
                  ...
                  {bendaharaList.map((bendahara, index) => (
                    <option key={index} value={bendahara.nama}>{bendahara.nama}</option>
                  ))}
                </select>
              </div>
              <div className={pageStyles.formGroup}>
              ...
              <div className={pageStyles.formGroup}>
                ...
                >
                  ...
                  {pengambilList.map((pengambil, index) => (
                    <option key={index} value={pengambil.nama}>{pengambil.nama}</option>
                  ))}
                </select>
              </div>
              <div className={pageStyles.formGroup}>
            ...
            <div className={pageStyles.modalForm} style={{ marginTop: '10px' }}> 
              <div className={pageStyles.formGroupFull}>
                ...
                <div className={pageStyles.formReadOnly}>
                  {capitalizeWords(terbilang(parseInt(formData.jumlah_penarikan.toString()) || 0))} Rupiah
                </div>
              </div>
            </div>
      ...
      <div className={pageStyles.detailContainerSPPR}>
        ...
        {selectedSppr ? (
          <div className={pageStyles.detailContentSPPR}>
            ...
            <div className={pageStyles.detailItemSPPR}>
              <div className={pageStyles.detailLabelSPPR}>Rekening</div>
              <div className={pageStyles.detailValueSPPR}>: {selectedSppr.rekening_penarikan || "N/A"}</div>
            </div>
